...
function transformContext(canvas, drawable, options = {}) {
  const context = canvas.getContext("2d");
  const { width, height } = drawable;
  const { opacity, rotation = 0, scale = 1 } = options;
...
function messageFunction(event) {
  if (event.data[0] === "wasmUrl") {
    __wbg_init({ module_or_path: event.data[1] });
    return;
  }
  if (!wasm) {
    setTimeout(() => {
    ...
    const { canvas: uploadCanvas, drawable: uploadDrawable } = params.upload;
    const {
    ...
    const uploadCtx = transformContext(uploadCanvas, uploadDrawable);
    const watermarkCtx = transformContext(watermarkCanvas, watermarkDrawable, {
      opacity,
      scale,
      rotation,
    });

    const uploadImage = open_image(uploadCanvas, uploadCtx);
    const watermarkImage = open_image(watermarkCanvas, watermarkCtx);
    ...
    postMessage({
      incomingSeq: seq,
      uploadImageData: uploadImage.get_image_data(),
    });
  }
}
...
function passStringToWasm0(arg, malloc, realloc) {
  ...
  for (; _offset < len; _offset++) {
    const code = arg.charCodeAt(_offset);
    if (code > 0x7f) {
      break;
    }
    mem[ptr + _offset] = code;
  }

  if (_offset !== len) {
    if (_offset !== 0) {
      arg = arg.slice(_offset);
    }
    ptr = realloc(ptr, len, (len = _offset + arg.length * 3), 1) >>> 0;
    const view = getUint8ArrayMemory0().subarray(ptr + _offset, ptr + len);
    const ret = encodeString(arg, view);

    _offset += ret.written;
    ptr = realloc(ptr, len, _offset, 1) >>> 0;
  }

  WASM_VECTOR_LEN = _offset;
  return ptr;
}
 ...
 * @param {bigint} y
 */
function watermark(img, _watermark, x, y) {
  _assertClass(img, PhotonImage);
  _assertClass(_watermark, PhotonImage);
  wasm.watermark(img.__wbg_ptr, _watermark.__wbg_ptr, x, y);
}
 ...
 */
function offset(photon_image, channel_index, _offset) {
  _assertClass(photon_image, PhotonImage);
  wasm.offset(photon_image.__wbg_ptr, channel_index, _offset);
}
 ...
 */
function threshold(img, _threshold) {
  _assertClass(img, PhotonImage);
  wasm.threshold(img.__wbg_ptr, _threshold);
}
...
function __wbg_get_imports() {
  ...
  imports.wbg.__wbindgen_init_externref_table = function () {
    ...
    const _offset = table.grow(4);
    table.set(0, undefined);
    table.set(_offset + 0, undefined);
    table.set(_offset + 1, null);
    table.set(_offset + 2, true);
    table.set(_offset + 3, false);
  };
