...
const QrCodeGen = (function () {
  ...
  /*export*/ class WasmQrCode {
    /**
     * Rasterize an SVG string to PNG at the given pixel width.
     * Height follows the SVG's aspect ratio (square for our QR styles).
     * @param {string} svg
     * @param {number} width
     * @returns {Uint8Array}
     */
    static svg_to_png(svg, width) {
      try {
        ...
        wasm.wasmqrcode_svg_to_png(retptr, ptr0, len0, width);
        let r0 = getDataViewMemory0().getInt32(retptr + 4 * 0, true);
        let r1 = getDataViewMemory0().getInt32(retptr + 4 * 1, true);
        let r2 = getDataViewMemory0().getInt32(retptr + 4 * 2, true);
        if (r2) {
          throw takeObject(r1);
        }
  ...
  return { WasmQrCode, initSync, init: __wbg_init };
})();
